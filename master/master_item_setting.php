...
$form_action_return = 'master_plan_setting_list.php';
...
$p_condition = $_POST['condition'];		//有効無効
$p_item      = $_POST['itemid'];		//費用登録
$p_detail    = $_POST['idetail'];		//費用費用詳細
$p_point     = $_POST['pointid'];		//ポイント登録
...
if($_POST['act'] == 'process'){
	...
	if(($err == false)OR(!$_POST['act_cancel'])){    //エラーなし
		...
		/*----- 登録情報 -----*/
		//▼値の設定
		$o_sum     = array_sum($p_item); 
		$js_item   = ($p_item)?   zToJSText($p_item)  : 'null';		//費用項目
		$js_detail = ($p_detail)? zToJSText($p_detail): 'null';		//費用費用詳細
		$js_point  = ($p_point)?  zToJSText($p_point) : 'null';		//ポイント項目
		
		$sql_data_array = array(
			'm_plan_sum'       => $o_sum,
			'm_plan_item'      => $js_item  ,
			'm_plan_detail'    => $js_detail,
			'm_plan_point'     => $js_point,
			'm_plan_condition' => $p_condition
		);
...
if($end == 'end'){
	...
	$p_use = '<input type="radio" name="condition" value="a" '.$checked_a.'> 有効　';
	$p_use.= '<input type="radio" name="condition" value="b" '.$checked_b.'> 無効'; 
	...
	//$d_item_ar＞設定した費用項目
	//▼単体費用
	$form_in     = mSetPlanSetting($item_ar,$detail_ar,$plan_id,$d_item_ar,$d_detail_ar,$input_control);
	...
	foreach($point_ar AS $kp => $vp){
		...
		$inp       = '<input class="input_text" type="text" name="pointid['.$kp.']" value="'.$v_point.'" '.$input_control.'>';
