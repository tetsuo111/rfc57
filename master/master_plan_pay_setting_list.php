...
$link_to = 'master_plan_pay_setting.php';
...
while($a = tep_db_fetch_array($query)) {
	...
	//▼内容取得
	$order_group = zSelectListSet($group_list,'','order_group','群なし','oGroup'.$a['id'],'');
	$condition   = ($a['condition'] == 'a')? '有効': '<span class="alert">無効</span>';
	$cl_cond     = ($a['condition'] != 'a')? 'class="no_use"':'';
	$operation   = '<a href="'.$link_to.'?m_plan_id='.$a['id'].'">合計の設定を編集する</a>';
